...
namespace cpp11
{
    ...
    public:
        ...
        CaffeineBeverage(std::function<int()> amountWaterMl, std::function<void()> brew)
            : m_brew(brew), m_amountWaterMl(amountWaterMl), m_description("CaffeineBeverage"), m_price(0.0f), m_condiment()
        {
        }

        CaffeineBeverage(std::function<int()> amountWaterMl, std::function<void()> brew, std::string description, float price)
            : m_brew(brew), m_amountWaterMl(amountWaterMl), m_description(description), m_price(price), m_condiment()
        {
        }
